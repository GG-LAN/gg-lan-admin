...
watch(() => props.modelId, id => {
    if (props.drawer.isVisible()) {
        axios.get(route("faqs.show.api", id))
        .then(({data}) => {
...
const submit = () => {
    form.put(route("faqs.update", props.modelId), {
        preserveScroll: true,
        onSuccess: () => {
            props.drawer.hide();
        }
...
<template>
    <BoxDrawer :uid="'drawer-update-' + props.uid" :title="props.title" :drawer="props.drawer">
        <form @submit.prevent="submit">
            <div class="space-y-4">
                <div>
                    <InputLabel for="update-question" value="Question" />
                    <TextInput
                        id="update-question"
                        type="text"
                ...
                <div>
                    <InputLabel for="update-response" value="Réponse" />
                    <TextareaInput
                        id="update-response"
